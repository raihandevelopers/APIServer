...
let dbName = "pinkSurf" //'pinksurfing'
...
const readManyAsync = async(query, collectionName)=>{
    try{
        ...
        throw {error: error, message:'internal_server_error', status:'fail'}
    }
}
...
const updateOne = async(filter, query, collectionName) =>{
    Client.connect(mongoUrl,(async(err, client)=>{
        if (err){
            console.log(err)
            throw {error: err, message:"error_occured", status: 'fail'}
        }
        let db = await client.db("anxo")
        if(db !== undefined){
            let collection = await db.collection(collectionName)
           /*  let keys = Object.keys(query)
            ...
            return {error:'nil', message:'inserted_successfully', status: 'fail'}
        }
    }))
}
...
const readFromDB = async(query, collectionName)=>{
    Client.connect(mongoUrl,(async(err, client)=>{
        if (err){
            console.log(err)
            return new Error({error: err, message:"error_occured", status:'fail'})
        }
        let db = await client.db("anxo")
        if(db !== undefined){
            let collection = db.collection(collectionName)
            let result = await collection.findOne(query)/* .then(async result =>{
                console.log(result)
                return await result
            }).catch(err =>{
