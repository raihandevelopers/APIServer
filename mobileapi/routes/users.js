...
router.post('/register',validate('register'), async (req, res) => {
    ...
    let refcode
    if(referredCode != "") {
         refcode = await accountModel.findOne({ referralCode: referredCode})
        if(!refcode) {
            res.send({ status: false, message: 'referral code is wrong'})
        } else {
            userJSON.referredBy = refcode.wallets.mybiz
        }
    }
...
router.post('/confirm',validate('confirm'), async (req, res) => {
    ...
    try {
        ...
        let userses = await accountModel.findOne({email: email})
        let usdat 
        if(myCache.has(otp)){
            console.log('entry')
            usdat = myCache.get(otp)
        ...
        if (otp == usdat['otp'] && email == usdat['email']) { // remove second condition // TODO
            delete usdat['otp']
            // delete userData['expired']

            //generate wallets
            console.log(usdat.referredBy)
            let walletData = await createWallets(email, usdat)
...
router.post('/subscribe_contact',  async (req, res) => {
    ...
    router.get('/getOneUser', async (req, res)=>{
        await accountModel.findOne( { email:req.query.email }, function (err, found) {
            if(err) {
