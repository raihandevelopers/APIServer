...
async function transferCrypto(to, amount,currency,ref, req, res){
    if(currency != "btc"){
        try{
            ...
            await db.insert({to,amount,currency,ref}, "pendingTransactions")

            res.send({"status": 'success', message: 'Transaction Successful', error :"nil"})

        }catch(err){
            res.status(500).send({"status": 'fail', message: "Transaction failed", error :"err"})
        }
...
async function transfer(to, amount, currency, ref, req, res){
    ...
    if(currency == 'eth'){
        let tx = await web3Provider.eth.sendTransaction({
            ...
            res.status(200).send({"status": 'success', message: hash, error :"nil"})
            await db.insert({
        ...
        instance.methods.transfer(to, amount).send({from : wallet[0]}).on('transactionHash',async hash=>{
            res.status(200).send({"status": 'success', message: hash, error :"nil"})
            ...
            res.status(500).send( {"status": 'fail', message: "transaction_failed", error :err})
            await db.insert({
                email: req.user.email,
                ref: req.user.ref,
                source : currency,
