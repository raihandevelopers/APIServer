const request = require('request')
const requestPromise = require('request-promise')
module.exports = {
    post : async(method, params)=>{
        request(
          {
            method: 'POST',
            uri: 'http://arix_user:zijqexsa5fcd3964gh5kc3ljzetxhfx6@79.143.191.43:18444',
            headers: { 'content-type': 'text/plain' },
                  body: JSON.stringify({
                      method: method,
                      params: params,
                    // method: "omni_sendissuancefixed",
                    // params:["mnUvDVzQjPFiNkEg14jWf2GHZFQQwEQfsE",2,1,0,"","","testToken_omniT","","","100000"],
                  // method: "omni_createpayload_simplesend",
                  // params: [1, "100"],
                  //method: "omni_createpayload_issuancefixed",
                  //params:[1,1,0,"","","testToken1","","","10000"],
                  // method:"omni_sendrawtx",
                  // params:["mnUvDVzQjPFiNkEg14jWf2GHZFQQwEQfsE","", "","mnUvDVzQjPFiNkEg14jWf2GHZFQQwEQfsE","0.001"],
                  id: 'curltest',
                  jsonrpc: "1.0", 
              }),
            }
          , function (error, response, body) {
            console.log('response', error)
            if(error != null){
                console.log('code: '+ response.statusCode)
                console.log(body)
                return body
            } else {
                return {result: "error", error:body}
              }
            }
          )
    },

  postAsync: async (method, params) => {
    try{
      let response = await requestPromise(
        { method: 'POST'
        , uri: 'http://anxo:StS2020@79.143.191.43:6386',
        headers: { 'content-type': 'text/plain' },
              body: JSON.stringify({
                  method: method,
                  params: params,
                // method: "omni_sendissuancefixed",
                // params:["mnUvDVzQjPFiNkEg14jWf2GHZFQQwEQfsE",2,1,0,"","","testToken_omniT","","","100000"],
              // method: "omni_createpayload_simplesend",
              // params: [1, "100"],
              //method: "omni_createpayload_issuancefixed",
              //params:[1,1,0,"","","testToken1","","","10000"],
              // method:"omni_sendrawtx",
              // params:["mnUvDVzQjPFiNkEg14jWf2GHZFQQwEQfsE","", "","mnUvDVzQjPFiNkEg14jWf2GHZFQQwEQfsE","0.001"],
              id: 'curltest',
              jsonrpc: "1.0", 
          }),
        })
        //console.log('inside module',JSON.parse(response))
        return JSON.parse(response)
    }catch (err){
      console.log('err', JSON.parse(err.error))
      throw {
        error: 'error',
        reason: JSON.parse(err.error).error.message
      }
    }
       
            /* .then(res=>{
                console.log(res)
                return res
            }) .catch(err=>{
               
                throw err
            })*/
         /*  , function (error, response, body) {
            console.log('response', error)
            if(error != null){
                console.log('code: '+ response.statusCode)
                console.log(body)
                return body
            } else {
                return {result: "error", error:body}
              }
            } */
    },

    BCHpostAsync: async (method, params) => {
      try{
        let response = await requestPromise(
          { method: 'POST'
          , uri: 'http://anxo:StS2020@79.143.191.43:6388',
          headers: { 'content-type': 'text/plain' },
                body: JSON.stringify({
                    method: method,
                    params: params,
                  // method: "omni_sendissuancefixed",
                  // params:["mnUvDVzQjPFiNkEg14jWf2GHZFQQwEQfsE",2,1,0,"","","testToken_omniT","","","100000"],
                // method: "omni_createpayload_simplesend",
                // params: [1, "100"],
                //method: "omni_createpayload_issuancefixed",
                //params:[1,1,0,"","","testToken1","","","10000"],
                // method:"omni_sendrawtx",
                // params:["mnUvDVzQjPFiNkEg14jWf2GHZFQQwEQfsE","", "","mnUvDVzQjPFiNkEg14jWf2GHZFQQwEQfsE","0.001"],
                id: 'curltest',
                jsonrpc: "1.0", 
            }),
          })
          //console.log('inside module',JSON.parse(response))
          return JSON.parse(response)
      }catch (err){
        console.log('err', JSON.parse(err.error))
        throw {
          error: 'error',
          reason: JSON.parse(err.error).error.message
        }
      }
         
              /* .then(res=>{
                  console.log(res)
                  return res
              }) .catch(err=>{
                 
                  throw err
              })*/
           /*  , function (error, response, body) {
              console.log('response', error)
              if(error != null){
                  console.log('code: '+ response.statusCode)
                  console.log(body)
                  return body
              } else {
                  return {result: "error", error:body}
                }
              } */
      }
}